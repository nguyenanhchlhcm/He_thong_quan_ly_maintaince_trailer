...
export function AssignTireDialog({
  ...
  const filtered = useMemo(() =>
    warehouseTires.filter(t => {
      ...
      const isUsableStatus = t.trang_thai_vo === 'Chưa lắp' || (t.trang_thai_vo as string) === 'Chờ đắp' || !t.trang_thai_vo
              ...
              return (
                ...
                >
                  ...
                  <Badge className="shrink-0 bg-slate-700/50 text-slate-300 border-slate-600 text-[11px]">
                    {(tire.trang_thai_vo as string) === 'Chờ đắp' ? 'Chưa lắp' : (tire.trang_thai_vo || 'Kho')}
                  </Badge>
                </button>
              )
            })
